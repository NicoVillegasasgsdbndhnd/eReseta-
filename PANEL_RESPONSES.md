...
| **E** | No duplicate users / duplicate passwords | **Users:** `email` unique at the DB level. **Passwords:** now blocked per panel request — checked against existing bcrypt hashes on set, generic message, no stored fingerprint. | `create_users_table` migration (`->unique()`) · `app/Rules/UniquePasswordAcrossUsers.php` | "Duplicate **accounts** are impossible — email is `unique` at the **database** level. For **passwords**, we implemented the block you asked for: when a password is set we check it against the existing bcrypt hashes and reject a match with a **generic** message, *'choose a different password'*, so we never reveal whose password it is — and we store no extra fingerprint that would weaken the hashing. We document that the usual guidance is against de-duplicating passwords, which is why we did it the least-harmful way." |
...
1. **E — duplicate passwords (implemented per panel request).** Say you built it *and* understood the trade-off: check against bcrypt hashes on set, **generic** message so no password is leaked, **no** stored fingerprint so hashing isn't weakened, and a note that standard guidance is against it. **This shows you understand hashing** — you complied without blindly introducing the usual weakness.
